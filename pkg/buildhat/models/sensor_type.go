...
// getSystemMotorString returns the string for system motor types
func getSystemMotorString(st SensorType) string {
	switch st {
...
// getTechnicMotorString returns the string for Technic motor types
func getTechnicMotorString(st SensorType) string {
	switch st {
...
// getSpikeMotorString returns the string for SPIKE motor types
func getSpikeMotorString(st SensorType) string {
	switch st {
...
// getSpikeSensorString returns the string for SPIKE sensor types
func getSpikeSensorString(st SensorType) string {
	switch st {
	case SpikePrimeColorSensor:
		return "SPIKE Prime Color Sensor"
	...
	case SpikeEssential3x3ColorLightMatrix:
		return "SPIKE Essential 3x3 Color Light Matrix"
	default:
		return ""
	}
}

// getOtherSensorString returns the string for other sensor types
func getOtherSensorString(st SensorType) string {
	switch st {
	case None:
		return "None"
	case GeneralPwm:
	...
	case ColourAndDistanceSensor:
		return "Color and Distance Sensor"
	case MediumLinearMotor:
...
func (st SensorType) String() string {
	// Try each category
	if result := getSystemMotorString(st); result != "" {
		return result
	}
	if result := getTechnicMotorString(st); result != "" {
		return result
	}
	if result := getSpikeMotorString(st); result != "" {
		return result
	}
	if result := getSpikeSensorString(st); result != "" {
		return result
	}
	if result := getOtherSensorString(st); result != "" {
		return result
	}
